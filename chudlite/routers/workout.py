...
async def new_workout(
    ...
    workout = await workout_service.create_workout(db, name or None, date.today(), user.id)
...
async def workout_history(
    ...
    workouts = await workout_service.get_workout_history(db, user.id)
    return templates.TemplateResponse(
        request,
        "workout_history.html",
        {"workouts": workouts, "username": user.username},
    )


@router.get("/history/{workout_id}", response_class=HTMLResponse)
async def workout_history_detail(
    request: Request,
    workout_id: str,
    db: AsyncSession = Depends(get_session),
    user: User = Depends(get_current_user),
    ...
    if not workout or not workout.finished or workout.user_id != user.id:
        return RedirectResponse(url="/workout/history", status_code=303)
    return templates.TemplateResponse(
        request,
        "workout_history_detail.html",
        {"workout": workout, "username": user.username},
    )
...
async def workout_page(
    ...
    return templates.TemplateResponse(request, "workout.html", {"workout": workout, "username": user.username})
